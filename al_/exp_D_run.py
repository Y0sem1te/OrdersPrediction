#!/usr/bin/env python3
"""Experiment D: wider network + OneCycleLR warmup, reduced dropout and weight decay.
Train longer (50 epochs) to try to improve val accuracy.
"""
import os
import sys
import json
...
def run_experiment(epochs=50, batch_size=128, max_lr=1e-3):
    ...
    model = FBASModelD(len(fbas_to_idx), fbas_embedding_dim=32).to(device)
    ...
    optimizer = optim.AdamW(model.parameters(), lr=max_lr, weight_decay=1e-6)
    ...
    best_val_acc = 0.0
    best_path = OUT_DIR / 'exp_D_best_model.pt'
    ...
    for epoch in range(1, epochs+1):
        ...
        with torch.no_grad():
            for batch_feats, batch_labels in test_loader:
                ...
                val_pred_labels = (probs > 0.5).astype(float)
                val_correct += (val_pred_labels == batch_labels.cpu().numpy()).sum()
        ...
        print(f'Epoch {epoch}/{epochs} train_loss={train_loss:.4f} train_acc={train_acc:.4f} val_loss={val_loss:.4f} val_acc={val_acc:.4f} val_auc={val_auc:.4f}')
    ...
    results = {
        ...
        'final_test_auc': float(auc_score),
        'test_acc': float(test_acc),
        'metrics': metrics
    }

    out_json = OUT_DIR / 'exp_D_metrics.json'
    ...
    plt.legend()
    plt.subplot(1,3,3)
    plt.plot(metrics['val_auc'], label='val_auc')
    plt.title('val_auc')
    plt.legend()
    png = OUT_DIR / 'exp_D_metrics.png'
    plt.tight_layout()
    plt.savefig(str(png))

    print('\nExperiment D finished. best_val_acc=%.4f final_test_auc=%.4f test_acc=%.4f' % (best_val_acc, auc_score, test_acc))
...
if __name__ == '__main__':
    start = time.time()
    run_experiment(epochs=50, batch_size=128, max_lr=1e-3)
