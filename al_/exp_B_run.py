#!/usr/bin/env python3
"""Experiment B: embedding_dim=32, wider MLP (256-128-64) with BatchNorm, AdamW lr=5e-4
Saves metrics and plots under ai_peigui/al_.
"""
import os
import sys
import json
...
def run_experiment(epochs=30, batch_size=128):
    ...
    model = FBASModelExpB(len(fbas_to_idx), fbas_embedding_dim=32).to(device)
    ...
    best_val_acc = 0.0
    best_path = OUT_DIR / 'exp_B_best_model.pt'
    ...
    out_json = OUT_DIR / 'exp_B_metrics.json'
    ...
    plt.title('acc')
    plt.legend()
    png = OUT_DIR / 'exp_B_metrics.png'
    plt.tight_layout()
    plt.savefig(str(png))

    print('\nExperiment B finished. Best val acc=%.4f, test acc=%.4f' % (best_val_acc, test_acc))
...
if __name__ == '__main__':
    start = time.time()
    run_experiment(epochs=30, batch_size=128)
